...
bool RpcServer::Start(int32_t thread_num, const char* addr, const char* port) {
    ...
    DB_LOG(INFO, "rpc server start info, thread pool num: %d, addr: %s, port: %s", 
...
void* RpcServer::RpcProcessor(void *arg) {
    ...
    if (method_iter == method_hashmap.end() || NULL == method_iter->second) {
        DB_LOG(ERROR, "find hash code failed! request hash code is: %u", hash_code);
    ...
    if (!rpc_serv_ptr->SendFormatStringMsg(event_fd, response)) {
        DB_LOG(ERROR, "send format response failed!");
...
bool RpcServer::GetMethodRequest(int32_t event_fd, RpcMessage& recv_rpc_msg) {
    ...
    if (connection_manager_ptr_->EpollRecvMsg(event_fd, msg_str) < 0) {
        DB_LOG(ERROR, "connection_manager recv msg failed!");
        return false;
    }
    ...
    if (!recv_rpc_msg.ParseFromString(msg_str)) {
        DB_LOG(ERROR, "parse from string msg failed!");
        return false;
    }
    return true;
}
...
bool RpcServer::SendFormatStringMsg(int32_t event_fd, Message* response) {
    ...
    if (!response->SerializeToString(&response_str)) {
        DB_LOG(ERROR, "response_str SerializeToString failed!");
        return false;
    }
    ...
    if (!send_rpc_msg.SerializeToString(&send_str)) {
        DB_LOG(ERROR, "send_str SerializeToString failed!");
        return false;
    }
