...
bool ConnectionManager::Initialize() {
    ...
    if (setrlimit(RLIMIT_NOFILE, &rt) == -1) {
        DB_LOG(ERROR, "set setrlimit falied size %d", MAXEPOLLSIZE);
        return false;
    }
    return true;
}
...
int32_t ConnectionManager::TcpListen(
    ...
    if ((getaddrinfo(set_host_, set_port_, &hints, &res)) != 0) {  
        DB_LOG(ERROR, "tcp_connect error for %s, %s", set_host_, set_port_);
        return -1;
    }
...
int32_t ConnectionManager::SendMsg(int32_t fd, string& send_msg_str) {
    ...
    do {
        ...
        if (buf_len < 0) {
            if (EINTR == errno) {
                DB_LOG(ERROR, "send error errno is EINTR");
                return -1;
            } 
            if (EAGAIN == errno) {
                sleep(1);
...
int32_t ConnectionManager::EpollInit(int32_t listenfd) {
    ...
    if (epoll_ctl(ep_create_fd_, EPOLL_CTL_ADD, listenfd, &ev_) < 0) {
        DB_LOG(ERROR, "epoll set insertion error: fd = %u", listenfd);
        return -1;
    }
    return 0;
}
...
int32_t ConnectionManager::EpollWait(int32_t max_events, struct epoll_event* events) {
    ...
    if (ready_nfds < 0) {
        DB_LOG(ERROR, "epoll wait error return fd is %d\n", ready_nfds);
        return -1;
    }
    return ready_nfds;
...
bool ConnectionManager::SetNonBlock(int32_t sock) {
    ...
    if (opts < 0) {
        DB_LOG(ERROR, "set non block failed! fcntl(sock,GETFL).");
        return false;
    }
...
bool ConnectionManager::ConfigSet(const char* host, const char* port) {
    ...
    if (conf_host_size > SOCKINFO_MAXSIZE || param_host_size > SOCKINFO_MAXSIZE) {
        DB_LOG(ERROR, "invalid host  conf host: %s, param host: %s\n",  conf_host_, host);
        return false;
    }
    if (conf_port_size > SOCKINFO_MAXSIZE || param_port_size > SOCKINFO_MAXSIZE) {
        DB_LOG(ERROR, "invalid port  conf port: %s, param port: %s\n", conf_port_, port);
        return false;
    }
