...
void Channel::CallMethod(const MethodDescriptor* method,
    ...
    if (connect_fd_ < 0) {
        DB_LOG(ERROR, "rpc connect server failed!");
        return;
    }
    ...
    if (SendMsg(connect_fd_, send_str) < 0) {
        DB_LOG(ERROR, "send msg error!");
    }

    string recv_str;
    if (RecvMsg(connect_fd_, recv_str) < 0) {
        DB_LOG(ERROR, "rcv msg error!");
    }
    close(connect_fd_);

    if (!FormatRecvMsg(recv_str, response)) {
        DB_LOG(ERROR, "format recv msg failed!");
    }
}
...
bool FormatSendMsg(
    ...
    if (NULL == request) {
        DB_LOG(ERROR, "request is null ptr!");
        return false;
    }

    string request_str;
    if (!request->SerializeToString(&request_str)) {
        DB_LOG(ERROR, "request SerializeToString has failed!");
        return false;
    }
    RpcMessage rpc_msg;
...
bool FormatRecvMsg(const string& recv_str, Message* response) {
    ...
    if (!recv_rpc_msg.ParseFromString(recv_str)) {
        DB_LOG(ERROR, "parse recv msg error! %s", recv_str.c_str());
        return false;
    }

    if (500 == recv_rpc_msg.head_code()) {
        DB_LOG(ERROR, "server internal error!");
    }

    if ("0" != recv_rpc_msg.body_msg() && 
        !response->ParseFromString(recv_rpc_msg.body_msg())) {
        DB_LOG(ERROR, "parse recv body msg error!");
        return false;
    }
    return true;
}
